...
func TestE2E_Tool_ToolsList_ShowsRequirements(t *testing.T) {
	...
	if !strings.Contains(out, "(requires: gorm)") {
		t.Errorf("`crank tools` should annotate migrate with gorm requirement:\n%s", out)
	}
}
...
// message instructing the user to install the gorm feature.
func TestE2E_Tool_Migrate_RequiresPostgres(t *testing.T) {
	...
	if !strings.Contains(out, "gorm") {
		t.Errorf("error should mention gorm, got:\n%s", out)
	}
}
...
func TestE2E_Tool_Migrate_NoMigrationsDir(t *testing.T) {
	dir := scaffold(t, "tool_migrate_no_dir", []string{"base", "gorm"})
...
// project has gorm enabled and a db/migrations/ dir, but no
...
func TestE2E_Tool_Migrate_NoDSN(t *testing.T) {
	dir := scaffold(t, "tool_migrate_no_dsn", []string{"base", "gorm"})
...
func TestE2E_Tool_Migrate_InvalidDatabaseURL(t *testing.T) {
	dir := scaffold(t, "tool_migrate_bad_url", []string{"base", "gorm"})
	out, err := runCrankRaw(t, "", "migrate", "up",
		"--project", dir,
...
func TestE2E_Tool_Migrate_DownDirection(t *testing.T) {
	dir := scaffold(t, "tool_migrate_down", []string{"base", "gorm"})
...
func TestE2E_Tool_Migrate_StepsFlag(t *testing.T) {
	dir := scaffold(t, "tool_migrate_steps", []string{"base", "gorm"})
